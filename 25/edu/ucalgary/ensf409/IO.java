...
public class IO{
    ...
    public void start(){
        ...
        if(combFound == false){ //if no combination is found, print recommedations
            test.printRecommedations();
            test.close();
            System.exit(1);
        }
        else{ //if combination found ask user if they happy with it
            allIds = test.getFinalIds();
            allCosts = test.getFinalCost();
            sum = test.sumOfCosts();
            boolean answer = checkWithUser();
            if(answer == true){ //if user happy, text file made and update database
            ...
            else{ //if user unhappy, print recommedations
                test.printRecommedations();
                test.close();
                System.exit(1);
            }
