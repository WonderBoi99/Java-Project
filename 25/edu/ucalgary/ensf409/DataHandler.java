...
public class DataHandler {
	...
	//store total number of rows in table
	 ...
	 * initializes connection to databases
	 */	
    ...
    public void initializeConnection(){
    	try{
			...
			System.out.println("Failed in initializeConnection()");
    		e.printStackTrace();
    	}
    }
    ...
    public void close(){
        try{
			...
			System.out.println("Failed in close()");
            e.printStackTrace();
        }
    }
    ...
    public boolean findCombo(){
    	...
    	try{
			...
			System.out.println("Failed in findCombo()");
    		e.printStackTrace();    		
    	}	
    	return false;
    } 
	...
	/**
	 * finds and stores all possible combinations in allPossibleCombination attribute
	 */
	private void lookForAllCombination(){
		try{
			...
			System.out.println("Failed in lookForAllCombination()");
    		e.printStackTrace();    		
    	}	
	}
	...
	private void checklistForChair(){
		try{
			...
			System.out.println("Failed in checklistForChair()");
    		e.printStackTrace();    		
    	}
	}
	...
	private void checklistForDeskOrFiling(){
		try{
			...
			System.out.println("Failed in checlistForDeskOrFiling()");
    		e.printStackTrace();    		
    	}
	}
	...
	private void checklistForLamp(){
		try{
			...
			System.out.println("Failed in checklistForLamp()");
    		e.printStackTrace();    		
    	}
	}
	...
	public void deleteUsedIds(){
		try{
			String qe = "DELETE FROM " + inputCategory + " WHERE ID = ?"; //build query for deleting selected rows
			...
			for(int o = 0; o < finalIds.size(); o++){
				...
				for(int i = 0; i < tmp.size(); i++){ //finding which rows to delete
					myFinalStatment.setString(1, tmp.get(i));
					myFinalStatment.executeUpdate();	
				}
			}
			myFinalStatment.close(); //releasing
		}
		catch(SQLException e){
			System.out.println("Failed in deleteUsedIds()");
    		e.printStackTrace();    		
    	}
	}
	...
	private void checklist(String[] x){
		try{
			...
			System.out.println("Failed in checklist()");
			e.printStackTrace();    		
		}
	}
	...
	public void printRecommedations(){
		try{
			...
			System.out.println("Failed in printRecommedations()");
			e.printStackTrace();    		
		}
	}
	...
	public void makeFile(){
		try{   
			...
			System.out.println("Failed in makeFile()");
			e.printStackTrace();
		}
	}	
}
